            ...
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center">
                        ...
                        <div class="col-10">
                            <p class="my-0"><?= $this->view->produto['nome'] ?>
                                <br><small><?= $this->view->produto['data_anunciada'] ?></small>
                                <?php if ($this->view->produto['data_vencimento'] !== "NULL") { ?>
                                    <br><small>Disponível até: <?= date_format(date_create($this->view->produto['data_vencimento']), "d/m/Y") ?></small>
                                <?php } ?>
                            </p>
                        </div>
                    </div>
                ...
                <div class="card-body pb-0">
                    <div class="row g-4 align-items-start">
                        ...
                        <div class="col-md-8">
                            <div>
                                <h5 class="card-title"><?= $this->view->produto['titulo'] ?></h5>
                                <div class="row my-3 text-center">
                                    <?php if ($this->view->produto['desconto'] && (strtotime($this->view->produto['data_desconto']) > time())) { ?>
                                        <div class="col-6" title="preço">
                                            <i class="fa-solid fa-dollar-sign"></i>
                                            <small>R$</small>
                                            <span class="text-danger">
                                                <s><?= preg_replace("/(\d)(?=(\d{3})+(?!\d))/", "$1.", str_replace(".", ",", $this->view->produto['valor'])) ?></s>
                                            </span>
                                        </div>
                                        <div class="col-6" title="promoção">
                                            ...
                                            <span class="text-success">
                                                <?= preg_replace("/(\d)(?=(\d{3})+(?!\d))/", "$1.", str_replace(".", ",", $this->view->produto['valor'] - $this->view->produto['desconto'])) ?>
                                            </span>
                                        </div>
                                    <?php } else { ?>
                                        <div class="col-6" title="preço">
                                            <i class="fa-solid fa-dollar-sign"></i>
                                            <small>R$</small>
                                            <span class="text-success"><?= preg_replace("/(\d)(?=(\d{3})+(?!\d))/", "$1.", str_replace(".", ",", $this->view->produto['valor'])) ?></span>
                                        </div>
                                    <?php } ?>
                                </div>
                                <div>
                                    <p>
                                        <small>Quantidade: <?= $this->view->produto['quantidade'] ?></small>
                                        <br>
                                        <small>Estado: <?= ucfirst($this->view->produto['estado']) ?></small>
                                        <br>
                                        <small>Situação: <?= ucfirst($this->view->produto['operacao']) ?></small>
                                    </p>
                                </div>
                                <div class="mb-3">
                                    <p class="card-text justificado"><?= nl2br($this->view->produto['descricao']) ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-footer">
                    <div class="row g-2">
                        <?php if ($this->view->produto['cod_usuario'] !== $_SESSION['cod_usuario']) { ?>
                            ...
                            <div class="col-6 col-lg-4">
                                <a href="https://wa.me/55<?= preg_replace("/[^0-9]/", "", $this->view->produto['telefone']); ?>" target="_blank">
...
<script>
    ...
    function perguntar() {
        ...
        $.ajax({
            ...
            url: '/comentar_duvida',
            dataType: 'html',
            data: 'anuncio=' + <?= $this->view->produto['cod_anuncio'] ?> + '&texto=' + texto,
            beforeSend: function() {
    ...
    function duvidas() {
        $.ajax({
            ...
            url: '/duvidas',
            dataType: 'html',
            data: 'anuncio=' + <?= $this->view->produto['cod_anuncio'] ?>,
            success: function(data) {
    ...
    function avaliar() {
        ...
        $.ajax({
            ...
            url: '/avaliar',
            dataType: 'html',
            data: 'anuncio=' + <?= $this->view->produto['cod_anuncio'] ?> + '&texto=' + texto + '&avaliacao=' + classificacao_anterior,
            beforeSend: function() {
